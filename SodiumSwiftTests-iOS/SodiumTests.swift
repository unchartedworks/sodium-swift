...
class SodiumSwiftTests: XCTestCase {
    ...
    func testMapCalm() {
        ...
        var xs = [Bool]()
        do {
            let l = c.map{ $0 % 2 == 0 }.calm().listen{ xs.append($0) }
            //let l = c.calm().listen{ out.append($0) }
            defer { l.unlisten() }
            c.send(2)
            c.send(2)
        ...
        XCTAssert([true, false] == xs, "testMapCalm() failed \(xs)")
    }

    func testSnapshot() {
        let c       = CellSink<Int>(0)
        let trigger = StreamSink<Int64>()
        var xs      = [String]()
        let pair    = {(x: Int64, y: Int) -> String in String(x) + " " + String(y)}
        let append  = {(x: String) in xs.append(x)}
        let l = trigger.snapshot(c, f: pair).listen(handler: append)
        defer { l.unlisten() }
        trigger.send(100)
        c.send(2)
        trigger.send(200)
    ...
    func testCalm2() {
        ...
        var xs = [Int]()
        let l = Transaction.run({
            return c.calm().listen({ xs.append($0) })
        })
        c.send(4)
        c.send(2)
        c.send(4)
        ...
        defer { l?.unlisten() }
        let ys = [2, 4, 2, 4, 2]
        XCTAssert(xs == ys, "test() failed ys = \(xs)")
    }
    ...
    private struct Ss2 {
        let s: StreamSink<Int> = StreamSink<Int>()
    }

    func testSwitchSSimultaneous() {
        let ss1 = Ss2()
        let css = CellSink<Ss2>(ss1)
        let so  = Cell<Int>.switchS(css.map({$0.s}))
        var xs  = [Int]()
        let l   = so.listen(handler: {xs.append($0)})
        defer { l.unlisten() }
        let ss2 = Ss2()
        let ss3 = Ss2()
        let ss4 = Ss2()
...
//    func testLiftList() {
//        let cellSinks = Array<Int>(0...50).map({x in CellSink<Int>(x)})
//        let sum       = cellSinks.map({ bi in bi.lift({v in v.sum()})
//        let xs        = [Int]()
//        let l         = sum.listen({xs.append($0)})
//        defer { l.unlisten() }
//        cellSinks[4].send(5)
//        cellSinks[5].send(5)
//        Transaction.runVoid({
